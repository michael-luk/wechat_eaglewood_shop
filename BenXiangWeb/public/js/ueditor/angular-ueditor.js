
/**
Created by 江湖一键客 2015-09-09
QQ:82530662
微信:JiangHuKeyKe
http://blog.csdn.net/error4o4
 */

(function() {
  "use strict";
  (function() {
    var NGUeditor;
    NGUeditor = angular.module("ueditor", []);
    NGUeditor.directive("ueditor", ['$timeout',
      function($timeout) {
        return {
          restrict: "C",
          require: "ngModel",
          scope: {
            config: "=",
            ready: "="
          },
          link: function($S, element, attr, ctrl) {
            var _NGUeditor, _updateByRender,_editorId;
            _editorId = attr.id ? attr.id : "_editor" + (Date.now());
            element[0].id = _editorId;
            _updateByRender = false;
            _NGUeditor = (function() {
              function _NGUeditor() {
                this.bindRender();  
                //this.initEditor();  
                return;
              }


              /**
               * 初始化编辑器
               * @return {[type]} [description]
               */

              _NGUeditor.prototype.initEditor = function() {
                var _UEConfig, _self;
                _self = this;
                if (typeof UE === 'undefined') {
                  console.error("Please import the local resources of ueditor!");
                  return;
                }
                _UEConfig = $S.config ? $S.config : {};
                //_editorId = attr.id ? attr.id : "_editor" + (Date.now());
                //element[0].id = _editorId;
                UE.getEditor(_editorId).destroy();
                this.editor = new UE.getEditor(_editorId, _UEConfig);
                //this.editor.isReady=false;
                return this.editor.ready(function() {
                  _self.editorReady = true;
                  _self.editor.addListener("contentChange", function() {
                    //console.log(_self.editor.getContent());
                    ctrl.$setViewValue(_self.editor.getContent());
                    if (!_updateByRender) {
                      if (!$S.$$phase) {
                        $S.$apply();
                      }
                    }
                    _updateByRender = false;
                  });
                  if(_self.modelContent){
                    if (_self.modelContent.length > 0) {
                      _self.setEditorContent();
                    }
                  }
                  if (typeof $S.ready === "function") {
                    $S.ready(_self.editor);
                  }
                });
              };

              _NGUeditor.prototype.setEditorContent = function(content) {
                if (content == null) {
                  content = this.modelContent;
                }
                if (this.editor && this.editorReady) {
                  this.editor.setContent(content);
                }
              };

              _NGUeditor.prototype.bindRender = function() {
                var _self;
                _self = this;
                ctrl.$render = function() {
                  _self.modelContent = (ctrl.$isEmpty(ctrl.$viewValue) ? "" : ctrl.$viewValue);
                  _updateByRender = true;
                  _self.setEditorContent();
                  function getE(){
                    var iid=setInterval(function(){
                        clearInterval(iid);
                        var ele=document.getElementById(_editorId);
                        if(ele){
                            _self.initEditor();
                        }else{
                            getE();
                        }
                    },1);
                  }
                  getE();
                };
              };
              return _NGUeditor;
            })();
            new _NGUeditor();
          }
        };
      }
    ]);
  })();

}).call(this);

//# sourceMappingURL=angular-ueditor.js.map
