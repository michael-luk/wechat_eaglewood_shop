    ...
    function ($scope, $http) {
        ...
        $scope.Jump = function (IndexNo) {
            if(!$scope.myOrder.orderProducts[IndexNo].hasCommented){
                window.location.href = window.location.protocol + '//' + window.location.host + '/w/evaluate?OrderId=' + $scope.myOrder.id + '&orderProductIndex='+ IndexNo +'&quantity=' + $scope.quantityList[IndexNo]
            }
        }
