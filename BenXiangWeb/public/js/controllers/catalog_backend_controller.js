...
var uploadTemplateCatalog = '<div> <input type="file" name="files[]" accept="image/*" ng-file-select="uploadImage($files, col.field, row.entity)"/>   <a class="fancybox" target="_blank" data-fancybox-group="gallery" fancybox ng-if="isShowImg(COL_FIELD)" ng-href="/showImage/{{COL_FIELD}}"><img ng-src="/showimg/thumb/{{COL_FIELD}}" style="width:120px;height:80px" ></a></div>';
var uploadTemplateCatalogSmallIamge = '<div> <input type="file" name="files[]" accept="image/*" ng-file-select="uploadImage($files, col.field, row.entity)"/>   <a class="fancybox" target="_blank" data-fancybox-group="gallery" fancybox ng-if="isShowImg(COL_FIELD)" ng-href="/showImage/{{COL_FIELD}}"><img ng-src="/showimg/thumb/{{COL_FIELD}}" style="width:80px;height:80px" ></a></div>';
...
app.controller('CatalogBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    $scope.gridCatalog = { data: 'list',
        ...
        columnDefs: [
            ...
            {field: 'description', displayName: '系列描述', width: '15%', enableCellEdit: true, editableCellTemplate: cellEditableTemplate},
