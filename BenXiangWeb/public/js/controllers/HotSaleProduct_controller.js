...
app.controller('HotSaleController', ['$scope', '$http', '$log', function ($scope, $http, $log) {
    ...
    function refreshDate() {
        // 拿热销产品
        $http.get('/products?isHotSale=true&size=10&page=' + $scope.page).success(function (data, status, headers, config) {
            if (data.flag) {
...
//										alert(data.message);
    ...
    $scope.find = function () {
        var url = '/products?isHotSale=true&size=1000&keyword=' + $scope.findProduct
