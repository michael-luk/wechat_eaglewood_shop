...
public class CatalogController extends Controller implements IConst {
	...
	public static Result getAllCatalogs(Integer page, Integer size, String orderBy, String sort) {
		...
		/*
		 * // 处理主题下产品数的问题 ObjectNode resultJson = (ObjectNode) Json.toJson(msg);
		 * JsonNode dataNode = resultJson.path("data"); for (int i = 0; i <
		 * msg.data.size(); i++) { ((ObjectNode)
		 * dataNode.get(i)).put("productNum", msg.data.get(i).products.size());
		 * }
		 */
	...
	//@Security.Authenticated(Secured.class)
	public static Result getCatalogProducts(long id, Integer page, Integer size) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		if (size == 0)
			size = PAGE_SIZE;
		...
		if (found != null) {
			if (found.products.size() > 0) {
				Page<ProductModel> records = ProductModel.find.where().eq("catalogs.id", found.id)
						.orderBy("isHotSale desc, soldNumber desc").findPagingList(size).setFetchAhead(false)
						.getPage(page - 1);
