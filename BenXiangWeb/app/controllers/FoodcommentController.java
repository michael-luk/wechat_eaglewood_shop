...
public class FoodcommentController extends Controller implements IConst {
	...
	public static Result getAll(Long productId, Integer page, Integer size) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		if (size == 0)
			size = PAGE_SIZE;
		...
		if (productId > 0) {
			records = FoodcommentModel.find.where().eq("product.id", productId).orderBy("id desc").findPagingList(size)
					.setFetchAhead(false).getPage(page - 1);
		} else {
			records = FoodcommentModel.find.orderBy("id desc").findPagingList(size).setFetchAhead(false)
					.getPage(page - 1);
		}
