...
public class ProductController extends Controller implements IConst {
	...
	public static Result getAllProducts(Long catalogId, String keywordProperty, String keyword, boolean isHotSale, boolean isActiveProduct, Integer page, Integer size, String orderBy, String sort) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		if (size == 0)
			size = PAGE_SIZE;
		...
		if (StrUtil.isNull(orderBy))
			orderBy = "isHotSale desc, soldNumber desc, isActiveProduct desc";
		else {
		...
		if (catalogId > 0) {
			...
			if (StrUtil.isNull(keyword)) {
				records = ProductModel.find.orderBy(orderBy).findPagingList(size).setFetchAhead(false)
						.getPage(page - 1);
				if (isHotSale) {
					records = ProductModel.find.where().eq("isHotSale", true).orderBy(orderBy).findPagingList(size)
							.setFetchAhead(false).getPage(page - 1);
				}
				...
				if (StrUtil.isNull(keywordProperty)) {
					records = ProductModel.find.where().like("name", "%" + keyword + "%").orderBy(orderBy)
							.findPagingList(size).setFetchAhead(false).getPage(page - 1);
				} else {
					records = ProductModel.find.where().like(keywordProperty, "%" + keyword + "%").orderBy(orderBy)
							.findPagingList(size).setFetchAhead(false).getPage(page - 1);
				}
			}
		}
	...
	public static Result updateProduct(long id) {
		...
		if (!httpForm.hasErrors()) {
			...
			found.descriptionImages = formObj.descriptionImages;
